...
export class UserFullName {
  ...
  private static MAX_LENGTH: number = 35;
  ...
  public static create(firstName: string, lastName: string): UserFullName {
    firstName = (firstName ?? "").trim();
    lastName = (lastName ?? "").trim();
    return new UserFullName(firstName, lastName);
  }

  public static createv2(firstName: string, lastName: string): Result<AppNotification, UserFullName> {
    ...
    return Result.ok(new UserFullName(firstName, lastName));
  }
}